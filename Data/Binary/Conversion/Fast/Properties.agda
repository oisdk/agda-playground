...
tail𝔹-inc : ∀ xs → map inc (tail𝔹 (inc xs)) ≡ tail𝔹 (inc (inc (inc xs)))
tail𝔹-inc 0ᵇ = refl
...
tail-homo : ∀ n → tail𝔹 (inc ⟦ n ⇑⟧) ≡ just ⟦ n ÷ 2 ⇑⟧
tail-homo n = go n ; cong (just ∘ ⟦_⇑⟧) (sym (div-helper-lemma 0 1 n 1))
  where
  go : ∀ n → tail𝔹 (inc ⟦ n ⇑⟧) ≡ just ⟦ div-helper′ 1 n 1 ⇑⟧
  go zero = refl
  go (suc zero) = refl
  go (suc (suc n)) = sym (tail𝔹-inc ⟦ n ⇑⟧) ; cong (map inc) (go n)
...
head-tail-cong (1ᵇ xs) (1ᵇ ys) h≡ t≡ = cong 1ᵇ_ (just-inj t≡)
...
head-tail-cong (2ᵇ xs) (2ᵇ ys) h≡ t≡ = cong 2ᵇ_ (just-inj t≡)

lemma₁ : ∀ x xs → head𝔹 (if x then 1ᵇ xs else 2ᵇ xs) ≡ just x
lemma₁ false xs = refl
lemma₁ true xs = refl

lemma₂ : ∀ x xs → tail𝔹 (if x then 1ᵇ xs else 2ᵇ xs) ≡ just xs
lemma₂ false xs = refl
lemma₂ true xs = refl
...
fast-correct-helper (suc n) (suc w) p =
  ...
  head-tail-cong _ (inc ⟦ n ⇑⟧)
    ...
    (lemma₂ (rem n 2 ≡ᴮ 0) (F.toBin-helper (n ÷ 2) w) ; cong just (fast-correct-helper (n ÷ 2) w (div2≤ n w (≤-pred-pred n w p))) ; sym (tail-homo n))
